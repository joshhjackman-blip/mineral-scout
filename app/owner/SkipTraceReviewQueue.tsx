...
export default function SkipTraceReviewQueue({
  ...
  useEffect(() => {
    if (!selected) return
    setPhones((selected.phones ?? []).join('\n'))
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-6">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Leads where skip trace returned no phone. Look the number up, then enter it here.
            The CRM and shared cache update when you save.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-sm text-gray-400">
          No failed skip traces waiting for a number.
        </div>
      ) : (
            ...
            <table className="w-full min-w-[560px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['Owner', 'Address', 'County', 'Requested by', 'When'].map((h) => (
                    <th
                      key={h}
